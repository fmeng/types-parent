...
public final class JsonUtil {
    ...
    /**
     * 使用Class参数化类型, 不抛出异常，用于打印日志等不关心异常的场景
     *
     * @param json
     * @param clazz
     * @param <T>
    ...
    public static <T> T decodeQuietly(String json, Class<T> clazz) {
        try {
            ...
            logger.error("反序列化失败, json:{}, clazz:{}, reason:{}", json, clazz, e.getMessage(), e);
            return null;
        }
    }
    ...
    public static final SimpleModule ENUM_PARSER_MODULE;
    ...
    static {
        ...
        ENUM_PARSER_MODULE = new SimpleModule("fmeng-common"
                , new Version(1, 0, 0, null, "me.fmeng.utils", "fmeng-common"));
        OBJECT_MAPPER.registerModule(ENUM_PARSER_MODULE);
    }
}
